...
def xusermapzodimodel(lstar, tstar, rstar, num, inu, stepau, lambda, radin, pfunc500, im, Qabsuser, iras=iras, scatterflag=scatterflag, useralpha=useralpha, userdelta=userdelta, userdustsize=userdustsize, lambdaQabs = lambdaQabs, emit = emit, scaletoflux = scaletoflux):
#*****************************************************************
...
#temperature, then bnu at that position
   ...
   indices = [cube[[num-radin-3]:[num+radin+2][num-radin-3]:[num+radin+2] [num-radin-3]:[num+radin+2]] LT radin cnt].nonzero()	#
   indices = indices[0]
	= len(indices)
   if cnt != 0: 
	  im[indices] = 0.0
   ...
   if len(scaletoflux) == 2 AND scaletoflux[1] != lambda:
